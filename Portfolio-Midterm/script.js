...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileSlides.length > 1) {
        function showNextSlide() {
            ...
            profileSlides[currentSlideIndex].classList.remove('active');
            
            // Move to next slide (loop back to 0 if at the end)
            currentSlideIndex = (currentSlideIndex + 1) % profileSlides.length;
            
            // Add active class to new current slide
            profileSlides[currentSlideIndex].classList.add('active');
        }

        // Start the slideshow - change image every 3000ms (3 seconds)
...
function resizeCanvas() {
    ...
    // Redraw initial screen if game is not running
    if (!gameRunning) {
